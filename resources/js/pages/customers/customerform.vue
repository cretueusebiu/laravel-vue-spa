    ...
    <div class="row">
      <form  @submit.prevent="submitForm" @keydown="form.onKeyDown($event)">
        ...
        <div class="mb-3">
          ...
          <input id="customer_gender_male" v-model="form.gender" type="radio" value="male" class="form-check-input">
          <label class="form-check-label" for="customer_gender_male">Male</label>
          <input id="customer_gender_female" v-model="form.gender" type="radio" value="female" class="form-check-input">
        ...
        <div class="card-footer">
            ...
            <router-link :to="{ name : 'customers' }" class="btn btn-danger float-end">
              Cancel
            </router-link>
        </div>
        </div>
...
export default {
  ...
  methods: {
    loadData: function () {
      this.loading = true
      axios.get('/api/customers/' + this.customerId).then(({ data }) => {
    ...
    submitForm: function () {
      if (this.customerId) {
        this.form.put('/api/customers/' + this.customerId).then((response) => {
          this.$router.push({ name: 'customers' })
        }).catch((error) => {
          console.log(error)
        })
      } else {
        this.form.post('/api/customers').then((response) => {
          this.$router.push({ name: 'customers' })
        }).catch((error) => {
          console.log(error)
        })
